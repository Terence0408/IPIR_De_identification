...
get_cur.execute("Select row_id, subject_id, order_id, content, train from record_text;")# where subject_id= 109 and order_id =5;")
table = get_cur.fetchall()
...
for row in table:
    ...
    content = re.sub("\n"," ",re.sub("\+","",re.sub("\)", "",re.sub("\(", "", row[3]))))
    #print content[2850:2900]


    for phi_row in phi_table:
        content = re.sub(phi_row[3], phi_row[6], content,1)
        #print phi_row[3]+"_______________"+ content[2850:2900]
...
# phi location in sentence.

get_cur.execute("Select row_id, subject_id, order_id, clean_content from record_text;")# where subject_id= 388 and order_id =1;")
table = get_cur.fetchall()
